...
class Vacuum extends Observable {
  ...
  async clean() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (Clock.minutesFromStart() - start >= 15) break;
    }
    this.status = "off";
    this.battery -= 40;
    return true;
  ...
  async moveToRoom(room) {
    if (this.house.rooms[this.in_room].doors_to.includes(room)) {
      ...
      for (let i = 0; i < 10; i++) {
        await Clock.global.notifyAnyChange();
        if (Clock.minutesFromStart() - start >= 5) break;
      }
      this.in_room = room;
      return true;
    } else {
  ...
  async charge() {
    if (this.in_room !== this.base) return false;
    let chargeTime = 0.5 * (100 - this.battery);
    if (chargeTime < 0) chargeTime = 0;
    let start = Clock.minutesFromStart();
    this.house.utilities.electricity.consumption += 1;
    for (let i = 0; i < 10; i++) {
      await Clock.global.notifyAnyChange();
      if (Clock.minutesFromStart() - start >= chargeTime) break;
    }
    this.house.utilities.electricity.consumption -= 1;
    this.battery = 100;
    return true;
