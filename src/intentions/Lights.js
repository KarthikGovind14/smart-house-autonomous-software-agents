      ...
      let lightGoalPromise = new Promise(async (res) => {
        while (true) {
          ...
          this.agent.beliefs.declare(
            "light_on_override " + l.name,
            status == "on_override"
          );
          this.agent.beliefs.declare(
            "light_off_override " + l.name,
            status == "off_override"
          );
        }
      });
...
class SenseLightSensorsGoal extends Goal {}

class SenseLightSensorsIntention extends Intention {
  static applicable(goal) {
    return goal instanceof SenseLightSensorsGoal;
  }

  *exec({ sensors, min, max } = parameters) {
    var sensorsGoals = [];
    for (let p of sensors) {
      let sensorGoalPromise = new Promise(async (res) => {
        while (true) {
          ...
          if (val > max) {
            status = "high";
            this.log("sense: light_sensor " + p.name + " switched " + status);
...
class TurnOnLightIntention extends Intention {
  ...
  *exec({ light } = parameters) {
    if (
      this.agent.beliefs.check("light_on " + light.name) ||
      this.agent.beliefs.check("light_on_override " + light.name) ||
      this.agent.beliefs.check("light_off_override " + light.name)
    )
...
class TurnOffLightIntention extends Intention {
  ...
  *exec({ light } = parameters) {
    if (
      this.agent.beliefs.check("light_off " + light.name) ||
      this.agent.beliefs.check("light_on_override " + light.name) ||
      this.agent.beliefs.check("light_off_override " + light.name)
    )
...
class LightsIntensityIntention extends Intention {
  ...
  *exec({ lights, target } = parameters) {
    ...
    for (let l of lights) {
      let lightGoalPromise = new Promise(async (res) => {
        while (true) {
          let low = await this.agent.beliefs.notifyChange(
            "light_low " + l.name
          );
          let med = await this.agent.beliefs.notifyChange(
            "light_med " + l.name
          );
          let high = await this.agent.beliefs.notifyChange(
            "light_high " + l.name
          );
          await Promise.race([low, med, high]);
          let sensor = l.house.sensors.lights[l.name + "_light_sensor"];
          if (this.agent.beliefs.check("not light_on_override " + l.name)) {
            if (this.agent.beliefs.check("light_on " + l.name)) {
              l.setIntensity(l.intensity + target - sensor.value);
            } else {
              l.setIntensity(target - sensor.value);
            }
          }
        }
      });
...
class PersonOccupancyGoal extends Goal {}

class PersonOccupancyIntention extends Intention {
  static applicable(goal) {
    return goal instanceof PersonOccupancyGoal;
  }
      ...
      let heatingGoalPromise = new Promise(async (res) => {
        while (true) {
          let status = await this.agent.beliefs.notifyChange(
            "room_occupied " + l.name,
            "light"
          );
          let notOverridden =
            this.agent.beliefs.check("not light_on_override " + l.name) &&
            this.agent.beliefs.check("not light_off_override " + l.name);
          if (status) {
            //room occupied
            if (notOverridden) {
            ...
            if (notOverridden) {
              ...
              this.agent.postSubGoal(
                new ReturnToNormalGoal({ light: l, after: 10 })
              );
            }
          }
        }
